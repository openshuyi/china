    ...
    <main className="relative w-full max-w-[100vw] flex-1 space-y-4 px-2 py-4 md:px-4 lg:px-6 overflow-x-hidden">
      ...
      <section className="relative rounded-xl border border-fd-border/50 bg-fd-background/40 backdrop-blur-md p-4 md:p-6 text-center shadow-sm overflow-hidden w-full mx-auto">
        ...
        <p className="mx-auto mt-2 max-w-3xl text-sm text-fd-muted-foreground leading-relaxed">
          ...
          <span className="hidden md:inline"> 通过地理空间维度，探索中华文明形成的物质证据。</span>
        </p>
        ...
        <div className="mt-4 flex flex-wrap justify-center gap-3">
          ...
          >
            <span className="mr-1.5">在时间轴中探索历史演进</span>
            <svg aria-hidden="true" className="transition-transform group-hover:translate-x-1" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><title>前往时间轴</title><path d="M5 12h14"></path><path d="m12 5 7 7-7 7"></path></svg>
          </Link>
          <Link
            href="/artifacts"
            className="group relative inline-flex h-8 items-center justify-center overflow-hidden rounded-full border border-fd-border/60 bg-fd-card/80 px-4 text-xs font-medium text-fd-foreground transition-all duration-300 hover:scale-105 hover:border-fd-primary/40"
          >
            <span className="mr-1.5">查看文物库</span>
            <svg aria-hidden="true" className="transition-transform group-hover:translate-x-1" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><title>前往文物库</title><path d="M5 12h14"></path><path d="m12 5 7 7-7 7"></path></svg>
          </Link>
        </div>
      </section>
